...
public class AdventureBoss : MonoBehaviour
{
    [Header("Target")]
	...
	private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        if (other.gameObject.CompareTag("Bullet")) {
            ...
            if (hit == 5) {
                ...
                // Spawn ten more items
                for (int i = 0; i < 10; i++) {
                    GameObject newObject = Instantiate<GameObject>(prefabToSpawn);
                    newObject.transform.position = new Vector3(this.transform.position.x + 0.03f, this.transform.position.y - 0.04f, this.transform.position.z);
                }
            }
        }
    }
